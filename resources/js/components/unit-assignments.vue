<template>
  <div>
    ...
    <div class="container section box mx-auto" style="margin-top: 100px; width:85%;">
      <div class="row direction d-flex pl-3" style="margin-top:10px; padding: 5px;">
        <a style="color: #274F73;" href="/lecturer/">LECTURER HOME /</a>
        <a style="color: #274F73;" href="/lecturer/manage-assignments">&nbsp MANAGE ASSIGNMENTS /</a>
        <a style="color: #ff7350;" href="#">&nbsp {{unitCode}}</a>
      </div>
      <div class="assignment-show" style="margin-top: 50px;">
        <!----- Tab of units' name -------->
              ...
              <table class="table">
                ...
                <tbody>
                  <tr v-for="current in currentAssignments">
                    <td>
                      <a :href="'/lecturer/manage-assignments/assignment/'+current.id+'/info'">
                        {{ current.title }}
                      </a>
                    </td>
                ...
                <table class="table">
                  ...
                  <tbody>
                    <tr v-for="past in filteredOfferings">
                      <td>
                        <a :href="'/lecturer/manage-assignments/assignment/'+past.id+'/info'">
                          {{ past.title }}
                        </a>
                          <a :href="'/lecturer/manage-assignments/assignment/'+LessId">
                              {{ past.title }}
                          </a>
                      </td>
                      <td>{{ past.dueDate }}</td>
...
<script>
  ...
  export default {
    ...
    methods: {
      getUnitOfferings(){
        let endpoint = config.BASE_URL + "api/unit/assignment/offerings/"+this.unitCode
      ...
      getUnitAssignments() {
        let endpoint = config.BASE_URL + "api/unit/assignments/"+this.unitCode
